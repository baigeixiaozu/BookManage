...
<%
    ...
    List<Object> data = ms.queryISBN(curPage, count, ISBN);
...
<article class="markdown-body">
    <form name="thisform" method="get">
        ISBN：<input type="text" value="<%=ISBN%>" name="ISBN">
    ...
    <div class="page-nav">
        <c:if test="<%=curPage-1>0%>">
            <div>
                <a href="${pageContext.request.contextPath}/MarketingSystem/searchByISBN.jsp?page=<%=curPage-1%>&ISBN=<%=ISBN%>">上一页</a>
            </div>
        </c:if>
        <c:forEach begin="1" end="<%=pageCnt%>" varStatus="s">
            <div>
                ...
                <c:if test="${param.page!=s.index}">
                    <a href="${pageContext.request.contextPath}/MarketingSystem/searchByISBN.jsp?page=${s.index}&ISBN=<%=ISBN%>">${s.index}</a>
                </c:if>
            </div>
        </c:forEach>
        <c:if test="<%=(curPage!=pageCnt)%>">
            <div>
                <a href="${pageContext.request.contextPath}/MarketingSystem/searchByISBN.jsp?page=<%=curPage+1%>&ISBN=<%=ISBN%>">下一页</a>
            </div>
        </c:if>
    </div>
    <style>
